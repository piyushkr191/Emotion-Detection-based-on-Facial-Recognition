...
# Load model weights and initialize parameters
parameters = {
    "shape": [80, 80],
    "nbr_classes": 7,
    "batch_size": 8,
...
def time_feed():
    def generate():
        ...
        if success:
            ...
            if emotions:
                dominant_emotion = max(emotions, key=lambda x: x[1])[0]  # Emotion with highest confidence
                result = magnify_results(emotions)
                result += f"<p>Dominant Emotion: {emotions_with_smiley[dominant_emotion]}</p>"
                yield result
            else:
                yield "no faces found"
