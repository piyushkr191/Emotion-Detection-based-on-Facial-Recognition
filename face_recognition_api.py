...
def load_image_file(filename, mode='RGB'):
    ...
    if img.shape[0] > 800 or img.shape[1] > 800:
        baseheight = 500
        if img.shape[0] > img.shape[1]:  # taller image
            w = (baseheight / img.shape[0])
            new_width = int(img.shape[1] * w)
            new_size = (new_width, baseheight)
        else:  # wider image
            w = (baseheight / img.shape[1])
            new_height = int(img.shape[0] * w)
            new_size = (baseheight, new_height)

        img_pil = Image.fromarray(img)
        # img_resized = img_pil.resize(new_size, Image.ANTIALIAS)
